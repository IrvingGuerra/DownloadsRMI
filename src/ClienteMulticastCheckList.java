...
public class ClienteMulticastCheckList extends Thread{
    ...
    public ClienteMulticastCheckList(database db){
        this.db = db;
        System.out.print( ANSI_BLUE + "[ CREATED ] "+ANSI_RESET+" Cliente Multicast CheckList Creado. ");
    }
    
    public void run(){
        System.out.print( ANSI_GREEN + "[ INIT ] "+ANSI_RESET+" Cliente Multicast CheckList Iniciado");
        while(true){
            try{
